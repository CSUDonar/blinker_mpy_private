...
class BlinkerMQTT(MQTTProtocol):
    ...
    def checkMiotKA(self):
        ...
        if self.isMiotAlive is False:
            return False
        if (millis() - self.miotKaTime) < BLINKER_MQTT_KEEPALIVE:
            return True
        else:
            self.isMiotAlive = False
            return False
    ...
    def checkMiotCanPrint(self):
        print("checkMiotCanPrintcheckMiotCanPrint",self.checkMiotKA(),self.miotPrintTime)
        return True
        if self.checkMiotKA() is False:
            BLINKER_ERR_LOG("MQTT NOT ALIVE OR MSG LIMIT")
            return False
        if (millis() - self.miotPrintTime) >= BLINKER_MQTT_MSG_LIMIT or self.miotPrintTime == 0:
            return True
        BLINKER_ERR_LOG("MQTT NOT ALIVE OR MSG LIMIT")
        return False
    ...
    @classmethod
    def getInfo(cls, auth, aliType,miType, duerType):
        ...
        if aliType :
            url = url + aliType
        if miType :
            print("miTypemiTypemiType",miType)
            url = url + miType
...
class MQTTClients():
    ...
    def start(self, auth, aliType, miType,duerType):
        self.auth = auth
    ...
    def miotPrint(self, msg):
        print("miotPrintmiotPrintmiotPrint",msg,self.bmqtt.checkMiotCanPrint())
        if self.bmqtt.checkMiotCanPrint() is False:
            return
